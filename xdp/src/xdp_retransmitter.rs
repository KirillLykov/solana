...
/// [`FixedSrcXdpSender`] is a structure that simplifies sending packets over XDP with `XdpSender`
/// when source address is fixed for all items.
#[derive(Clone)]
pub struct FixedSrcXdpSender {
    sender: XdpSender,
    src_addr: SocketAddrV4,
}
...
impl XdpRetransmitBuilder {
    ...
    #[inline]
    pub fn build_with_src_addr(
        self,
        src_addr: SocketAddrV4,
    ) -> (XdpRetransmitter, FixedSrcXdpSender) {
        let (retransm, sender) = self.build();
        (retransm, FixedSrcXdpSender::new(sender, src_addr))
    }
}
