...
impl QuicXdpTxSocket {
    pub fn new(
        ...
        // Collect local interface IPs once at construction time.
        // We intentionally do not refresh them if interface addresses change later.
        // This is a low-risk tradeoff because local-destination egress is expected to be rare
        // (for example, RPC sendTransaction traffic or local testing).
        let local_ips = collect_local_ipv4_ips()
            .expect("Local interface IPs should be collected successfully.");
...
impl AsyncUdpSocket for QuicXdpTxSocket {
    fn create_io_poller(self: Arc<Self>) -> Pin<Box<dyn UdpPoller>> {
        ...
        // Ready but the selected channel in `IndexedXdpSender` is full. In this case, the try_send will
        // fail with `WouldBlock`, and the caller will call `poll_writable` again, which leads to
        // selecting the other channel in the next round.
