...
pub async fn refill_task(quotas: Arc<StreamQuotas>) {
    ...
    loop {
        ...
        let mut overflow = 0;
        total_tokens_to_refill = (overflow + base_fill_per_tick).min(base_fill_per_tick * 2);
        dbg!(total_tokens_to_refill);

        for entry in quotas.entries.iter() {
            let my_fraction = total_tokens_to_refill * entry.stake / quotas.total_stake;
            let my_max_tokens = base_fill_per_tick * 2 * entry.stake / quotas.total_stake;
