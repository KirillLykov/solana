...
) -> Result<(NodeAddressProvider, WebsocketNodeAddressService), Error> {
    let (websocket_slot_event_stream, ws_task_handle) = websocket_slot_event_stream(websocket_url);
    let (provider, service) =
        NodeAddressService::run(rpc_client, websocket_slot_event_stream, config, cancel).await?;

    Ok((
        provider,
        WebsocketNodeAddressService {
            service,
